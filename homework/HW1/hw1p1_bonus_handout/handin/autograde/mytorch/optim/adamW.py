...
class AdamW():
    ...
    def step(self):
        ...
        for layer_id, layer in enumerate(self.l):
            ...
            self.m_W[layer_id] = self.beta1 * self.m_W[layer_id] + \
                (1 - self.beta1) * dLdW
            ...
            self.m_b[layer_id] = self.beta1 * self.m_b[layer_id] + \
                (1 - self.beta1) * dLdb
            ...
            layer.W = layer.W - self.lr * (m_W_hat / np.sqrt(v_W_hat + self.eps) + self.weight_decay * layer.W)
            layer.b = layer.b - self.lr * (m_b_hat / np.sqrt(v_b_hat + self.eps) + self.weight_decay * layer.b) 
