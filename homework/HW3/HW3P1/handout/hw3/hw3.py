...
def inference(net, inputs):
    ...
    num_classes, hidden_dim = net.projection.W.shape
    logits = np.zeros(shape=(seq_len, num_classes))
    hnext = np.zeros(shape=(seq_len, hidden_dim))
    for t in range(seq_len):
        if t == 0:
            h_prev = np.zeros(shape=(hidden_dim))
        else:
            h_prev = hnext[t - 1]
        logits[t], hnext[t] = net.forward(inputs[t], h_prev)
