...
class RNNPhonemeClassifier(object):
    ...
    def forward(self, x, h_0=None):
        ...
        for t in range(seq_len):
            ...
            for l in range(self.num_layers):
                ...
                if l == 0:
                    ...
                    layer_input = hidden[l - 1]
                
                hidden[l] = self.rnn[l].forward(layer_input, h_prev)
            self.hiddens.append(hidden.copy())
        ...
        logits = self.output_layer.forward(final_hidden)
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for t in reversed(range(seq_len)):
            for l in reversed(range(self.num_layers)):
                ...
                h_prev_time = self.hiddens[t][l]

                if l == 0:
                    x_input = self.x[:, t, :]
                else:
                    x_input = self.hiddens[t+1][l-1]
                
                dx, dh_prev = self.rnn[l].backward(dh[l], h_t, x_input, h_prev_time)
