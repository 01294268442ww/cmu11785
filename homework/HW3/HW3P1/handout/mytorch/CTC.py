...
class CTC(object):
    ...
    def extend_target_with_blank(self, target):
        ...
        for i in range(2, N - 1):
            if (
                extended_symbols[i] != self.BLANK
                and extended_symbols[i] != extended_symbols[i - 2]
            ):
                skip_connect[i] = 1
    ...
    def get_posterior_probs(self, alpha, beta):
        ...
        gamma /= np.expand_dims(sumgamma, axis=1)
...
class CTCLoss(object):
    ...
    def forward(self, logits, target, input_lengths, target_lengths):
        ...
        for batch_itr in range(B):
            ...
            extended_symbols, skip_connect = self.ctc.extend_target_with_blank(batch_target)
            alpha = self.ctc.get_forward_probs(batch_logits, extended_symbols, skip_connect)
            beta = self.ctc.get_backward_probs(batch_logits, extended_symbols, skip_connect)
            gamma = self.ctc.get_posterior_probs(alpha, beta)
            self.gammas.append(gamma)
            self.extended_symbols.append(extended_symbols)

            loss = -np.sum(gamma * np.log(batch_logits[:, extended_symbols]))
    ...
    def backward(self):
        ...
        for batch_itr in range(B):
            ...
            for t in range(self.input_lengths[batch_itr]):
                for r in range(len(extended_symbols)):
                    dY[t, batch_itr, extended_symbols[r]] -= self.gammas[batch_itr][t, r] / batch_logits[t, extended_symbols[r]]
