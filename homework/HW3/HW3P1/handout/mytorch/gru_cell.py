...
class GRUCell(object):
    ...
    def forward(self, x, h_prev_t):
        ...
        self.a = self.Wnx @ x + self.bnx + self.r * (self.Wnh @ h_prev_t + self.bnh)
        self.n = self.h_act(self.a)
    ...
    def backward(self, delta):
        ...
        dLdzt = delta * (self.hidden - self.n)
        dLdnt = delta * (1 - self.z)

        dn = dLdnt * self.h_act.backward(self.n)
        dz = dLdzt * self.z_act.backward()

        self.x = self.x.reshape(self.d, 1)
        self.hidden = self.hidden.reshape(self.h, 1)

        self.dWnx = dn.T @ self.x.T
        self.dbnx = np.sum(dn, axis=0)

        self.dWnh = dn.T * np.dot(self.r.reshape(-1, 1), self.hidden.T)
        self.dbnh = dn * self.r

        self.dWzx = dz.T @ self.x.T
        self.dbzx = np.sum(dz, axis=0)
        self.dWzh = dz.T @ self.hidden.T
        ...
        dx = self.Wnx.T @ dn.T + self.Wzx.T @ dz.T + self.Wrx.T @ dr
        # print(f"dx shape: {dx.shape}")
        dx = dx.T

        dh_prev_t = self.Wnh.T @ (self.r.reshape(1, -1) * dn).T + self.Wzh.T @ dz.T + self.Wrh.T @ dr + (delta * self.z).T
