...
class RNNCell(object):
    ...
    def backward(self, delta, h_t, h_prev_l, h_prev_t):
        ...
        dz = self.activation.backward(state=h_t)

        # 1) Compute the averaged gradients of the weights and biases
        self.dW_ih += (delta * dz).T @ h_prev_l / batch_size
        self.dW_hh += (delta * dz).T @ h_prev_t / batch_size
        self.db_ih += np.sum(delta * dz, axis=0) / batch_size
        self.db_hh += np.sum(delta * dz, axis=0) / batch_size

        # # 2) Compute dx, dh_prev_t
        dx        = (delta * dz) @ self.W_ih
        dh_prev_t = (delta * dz) @ self.W_hh
